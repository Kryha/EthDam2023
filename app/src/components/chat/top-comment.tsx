...
export const TopComment = ({ comment, ...rest }: Props) => {
  const { content, count = 0, id } = comment;
  ...
  const { data } = useQuery(
    ["count", id],
    async () => await callGetMessageById(id),
    { onSuccess, refetchInterval: 500 }
  );
  ...
  return (
    ...
    >
      <Stack direction="row" spacing={2} sx={{ p: 1 }}>
        <Avatar
          src={`https://api.dicebear.com/6.x/open-peeps/svg?seed=${content.username}`}
        />
        <Stack>
